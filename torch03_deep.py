...
x_raw = torch.FloatTensor([1, 2, 3]).unsqueeze(1).to(DEVICE)
y = torch.FloatTensor(y).unsqueeze(1).to(DEVICE)

########### Standard Scaling ############
mean = torch.mean(x_raw)
std = torch.std(x_raw)
x = (x_raw - torch.mean(x_raw)) / torch.std(x_raw)
#########################################
print('스케일링 후: ', x)
...
optimizer = optim.SGD(model.parameters(), lr=0.01) # Statistic Gradient Descent(경사하강법)
...
x_pred = (torch.Tensor([[4]]).to(DEVICE) - mean) / std
result = model(x_pred)
