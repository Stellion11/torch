...
class RNN(nn.Module):
    ...
    def forward(self, x):
        x, _ = self.rnn_layer1(x)   # 출력값이 2개(y 값, hidden space)
        x = self.relu(x)
        
        ##### 1. reshape : 잘 돌아가 
        # x = x.reshape(-1, 3*64)
        
        ##### 2. view: 에러 (view는 연속적 메모리 데이터에만 해당)
        # x = x.view(-1, 3*64) 
        
        ##### 3. contiguous를 이용해서 연속적인 메모리 라고 인식시킨다.
        # view를 쓰는게 더 빨르기 때문에 데이터가 커지면 view를 권장
        x = x.contiguous()
        x = x.view(-1, 3*64) 
